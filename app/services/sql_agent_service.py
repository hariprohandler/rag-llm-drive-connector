...
def create_sql_agent_executor(
    ...
    try:
        agent = create_sql_agent(
            ...
            toolkit=toolkit,
            verbose=True,
            agent_type="openai-tools",  # Best for structured output
            handle_parsing_errors=True,
            max_iterations=15,  # Limit iterations to prevent infinite loops
            early_stopping_method="force"  # Stop if agent gets stuck
        )
    except Exception:
        # Fallback to zero-shot-react-description if openai-tools not supported
        agent = create_sql_agent(
            llm=llm,
            toolkit=toolkit,
            verbose=True,
